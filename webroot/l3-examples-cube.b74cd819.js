...
var orbitControls = function (camera, domElement, renderer, scene) {
...
/* unused harmony default export */ var _unused_webpack_default_export = (OrbitControls);
...
var dat_gui_module = __webpack_require__(12);

// EXTERNAL MODULE: ./resources/threejs/controls/OrbitControls.ts
var OrbitControls = __webpack_require__(2);

// CONCATENATED MODULE: ./resources/threejs/libs/__global__.ts

var __global__ = window;
__global__.THREE = __global__.THREE || three_module;
/* harmony default export */ var _global_ = (__global__);

// CONCATENATED MODULE: ./resources/threejs/l3/examples/cube.ts
...
var run = function (canvas) {
    ...
    var camera = new three_module["PerspectiveCamera"](100, window.innerWidth / window.innerHeight, 0.1, 100);
    camera.position.set(0, 0, 35);
    var renderer = new three_module["WebGLRenderer"]({ canvas: canvas });
    renderer.setClearColor(0xeeeeee);
    renderer.shadowMap.enabled = true;
    // Plane
    var planeGeometry = new three_module["PlaneGeometry"](60, 40, 1, 1);
    var planeMaterial = new three_module["MeshLambertMaterial"]({ color: 0xffffff });
    ...
    scene.add(plane);
    // Light
    var ambientLight = new three_module["AmbientLight"](0x0c0c0c);
    scene.add(ambientLight);
    var spotLight = new three_module["SpotLight"](0xffffff);
    spotLight.position.set(-40, 60, -10);
    spotLight.castShadow = true;
    scene.add(spotLight);
    // Fog
    scene.fog = new three_module["FogExp2"](0xffffff, 0.015);
    var addCube = function () {
        var cubeSize = Math.ceil((Math.random() * 3));
        var cubeGeometry = new three_module["BoxGeometry"](cubeSize, cubeSize, cubeSize);
        var cubeMaterial = new three_module["MeshLambertMaterial"]({ color: Math.random() * 0xffffff });
        var cube = new three_module["Mesh"](cubeGeometry, cubeMaterial);
    ...
    var removeCube = function () {
        ...
        if (lastObject instanceof three_module["Mesh"]) {
            scene.remove(lastObject);
        }
    };
    renderer.render(scene, camera);
    _global_.scene = scene;
    _global_.addCube = addCube;
    _global_.removeCube = removeCube;
    for (var i = 0; i < 20; i++) {
        addCube();
    }
    var controls = {
        ...
        overrideMaterial: function () { return scene.overrideMaterial = !scene.overrideMaterial ? new three_module["MeshLambertMaterial"]({ color: 0xffffff }) : null; },
    };
    var datGUI = new dat_gui_module["a" /* GUI */]();
    ...
    var renderScene = function () {
        scene.traverse(function (e) {
            if (e instanceof three_module["Mesh"] && e !== plane) {
    ...
    renderScene();
    // OrbitCOntrols
    var disposeOrbitControls = Object(OrbitControls["a" /* orbitControls */])(camera, document.body, renderer, scene);
    return function () {
        datGUI.destroy();
        disposeOrbitControls();
    };
};
/* harmony default export */ var examples_cube = __webpack_exports__["default"] = (run);
